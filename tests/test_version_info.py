...
def test_user_facing_version_markers_match_v0423():
    root = Path(__file__).resolve().parents[1]
    assert 'Wersja wyświetlana: 0.4.23' in (root / 'README.md').read_text(encoding='utf-8')
    assert 'GPL-3.0-or-later' in (root / 'README.md').read_text(encoding='utf-8')
    assert 'ALO Music v0.4.23' in (root / 'START_ALO.bat').read_text(encoding='utf-8')
    assert 'Wersja <b>0.4.23</b>' in HELP_TOPICS['O programie']
