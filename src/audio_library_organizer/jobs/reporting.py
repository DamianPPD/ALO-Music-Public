...
def export_session_html(tracks: Iterable[TrackRecord], path: Path) -> Path:
    tracks = list(tracks)
    ...
    for track in tracks:
        ...
        review_rows.append(
            '<tr>'
            f'<td>{escape(name)}</td>'
            f'<td>{escape(track.status)}</td>'
            f'<td>{escape(", ".join(reasons))}</td>'
            '</tr>'
        )
    table_body = ''.join(review_rows) or '<tr><td colspan="3">Brak pozycji DO SPRAWDZENIA.</td></tr>'

    html_text = (
        '<!doctype html><html lang="pl"><head><meta charset="utf-8">'
        '<title>Raport sesji ALO Music</title><style>'
        ...
        '</style></head><body>'
        '<h1>Raport sesji ALO Music</h1><div class="muted">Podsumowanie bieżącej sesji biblioteki.</div>'
        '<div class="cards">'
        f'<div class="card"><b>{summary["total"]}</b>Wszystkie</div>'
        f'<div class="card"><b>{summary["ready"]}</b>Gotowe</div>'
        f'<div class="card"><b>{summary["duplicate"]}</b>Duplikaty</div>'
        f'<div class="card"><b>{summary["not_selected"]}</b>Nie wybieram</div>'
        f'<div class="card"><b>{summary["review"]}</b>Do sprawdzenia</div>'
        f'<div class="card"><b>{summary["manually_edited"]}</b>Ręcznie edytowane</div>'
        '</div><h2>DO SPRAWDZENIA — powody</h2>'
        '<table><thead><tr><th>Utwór</th><th>Status</th><th>Powód</th></tr></thead><tbody>'
